...
Да, там есть какие-то крышечки, но наверняка их можно убрать и найти нужную подстроку регулярным выражением, ведь регекспы в Perl могут все? Но как это сделать, где искать в документации?
...
Но когда, спустя некоторе время, я вернулся к своему посту (чтобы порадоваться растущему рейтингу), там меня озадачил комментарий :  

> «This is the wrong way to do it. You need to use a UCA match at level 1» **tchrist**

Итак, я узнал много новых слов при решении, но все равно сделал что-то неправильно! Хмм...

Я не имел никакого представления, что за зверь _UCA match at level 1_ и в чем моя ошибка, но твердо решил, что должен с этим разобраться. Особенно, после того, как посмотрел в профиле кто такой tchrist. Это оказался известный в Perl мире, разработчик Tom Christiansen, соавтор книг о Perl, один из разработчиков языка и, как оказалось позже, Unicode эксперт.

В результате исправленная версия выглядела чуть скучнее:
...
Рецепты – это неплохо. Это такая хорошая штука, которая позволят здорово экономить время. Например, готовка супа по рецепту отлично работает. И для многих случаев, с которыми приходится сталкиваться в программировании, тоже. Но это хорошо работает только для простых задач, а не для сложных!
